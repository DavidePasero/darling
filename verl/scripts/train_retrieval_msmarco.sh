#!/bin/bash
# MS MARCO Retrieval Training - Quality Only (No Diversity)
# Uses MS MARCO dataset directly from HuggingFace
...
PROJECT_NAME="msmarco_retrieval"
...
python3 -m verl.trainer.main_ppo \
    ...
    \
    `# Custom MS MARCO Dataset` \
    +data.custom_cls.path=verl.utils.dataset.msmarco_dataset \
    +data.custom_cls.name=MSMarcoRLDataset \
    +data.dataset_name=ms_marco \
    +data.dataset_config=v2.1 \
    +data.split=train \
    ...
    +reward_model.msmarco_dataset_name=ms_marco \
    +reward_model.msmarco_dataset_config=v2.1 \

...
class RewardModelWorker(Worker):
    ...
    def _build_model(self, config):
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if 'Athene' in local_path:
                reward_module = AtheneForSequenceClassification.from_pretrained(
                    pretrained_model_name_or_path=local_path,
                    config=model_config,
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                if 'Athene' in self.config.model.path:
                    output = self.reward_module(input_ids=input_ids_rmpad, attention_mask=None, position_ids=position_ids_rmpad) # prevent model thinks we are generating
                    reward_rmpad = output["scores"]
                    return reward_rmpad
                else:
    ...
    def _switch_chat_template(self, data: DataProto):
        ...
        for i in range(data.batch.batch_size[0]):
            ...
            if 'Athene' in self.config.model.path:
                prompt_with_chat_template = prompt_with_chat_template + target_tokenizer.cls_token
                if self.rank == 0 and i == 0:
                    print(f"Switch template. chat with cls token: {prompt_with_chat_template}")

            # the maximum length is actually determined by the reward model itself
            max_length = self.config.get("max_length", src_max_length)
            if max_length is None:
                max_length = src_max_length
                #max_length = 32768 # this is to make sure the CLS token does not get truncated

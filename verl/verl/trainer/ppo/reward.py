...
def get_custom_reward_fn(config, diversity=False):
    ...
    if diversity:
        reward_fn_config = config.reward_model.get("custom_diversity_function") or {}
    else:
...
def load_reward_manager(config, tokenizer, num_examine, diversity=False, **reward_kwargs):
    ...
    compute_score = get_custom_reward_fn(config, diversity=diversity)
    ...
    if compute_score is None:
        ...
        if sandbox_url:
            ...
            if config.reward_model.get("shape") == [1, -1]:
                final_compute_score = partial(_default_compute_score, shape=[1, -1])
            elif config.reward_model.get("shape") == [2, 1]:
                final_compute_score = partial(_default_compute_score, shape=[2, 1])
            else: # this is 1, 0
